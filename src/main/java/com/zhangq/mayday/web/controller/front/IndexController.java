...
import com.zhangq.mayday.model.enums.ArticleStatus;
import com.zhangq.mayday.model.enums.PageNumber;
import com.zhangq.mayday.model.enums.PostType;
import com.zhangq.mayday.service.*;
import com.zhangq.mayday.util.MaydayUtil;
...
public class IndexController extends BaseController {
	...
	public String post(Model model, @PathVariable(value = "articleUrl") String articleUrl, HttpServletRequest request) {
		...
		comment.setArticleId(articleCustom.getId());
        PageInfo<Comment> commentList=commentService.findPageComment(1,10,comment);
		for (int i = 0; i < commentList.getSize(); i++) {
			 Comment c=commentList.getList().get(i);
			PageInfo<Comment> clist=commentService.findAllChildComment(c);
			c.setCommentChildLists(clist.getList());
		}
